...
std::shared_ptr<Mesh> Mesh::create_cube(std::size_t width) {
    ...
    vertices.push_back({-s,  s,  s, 1.0f, 1.0f, 0.0f}); // желтый
    ...
    // Правая грань (x = s) - 2 треугольника
    ...
    vertices.push_back({ s, -s, -s, 0.0f, 1.0f, 1.0f}); // циан
    vertices.push_back({ s,  s, -s, 0.5f, 0.5f, 0.5f}); // серый
    vertices.push_back({ s,  s,  s, 0.0f, 0.0f, 1.0f}); // синий

    // Задняя грань (z = -s) - 2 треугольника
    ...
    vertices.push_back({-s, -s, -s, 1.0f, 0.0f, 1.0f}); // пурпурный
    vertices.push_back({-s,  s, -s, 1.0f, 1.0f, 1.0f}); // белый
    vertices.push_back({ s,  s, -s, 0.5f, 0.5f, 0.5f}); // серый

    // Левая грань (x = -s) - 2 треугольника
    ...
    // Верхняя грань (y = s) - 2 треугольника
    ...
    // Нижняя грань (y = -s) - 2 треугольника
